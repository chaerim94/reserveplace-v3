<template>
    <v-card style="width:450px; height:100%;" outlined>
        ...
        <v-card-text>
            ...
            <Number label="PlaceId" v-model="value.placeId" :editMode="editMode" :inputUI="''"/>
...
<script>
	...
	async created() {
        ...
        methods: {
            ...
            async reservationStatusUpdate() {
                try {
                    if(!this.offline) {
                        var temp = await axios.put(axios.fixUrl(this.value._links['reservationstatusupdate'].href))
